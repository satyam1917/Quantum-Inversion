#!/bin/bash
#SBATCH --job-name=qreg_cubic
#SBATCH --output=logs/qreg_cubic_%j.out
#SBATCH --error=logs/qreg_cubic_%j.err
...
"$PYTHON" src/run_quantum_cubic_regression.py \
    --n-points "${QREG_N_POINTS:-100}" \
    --train-ratio "${QREG_TRAIN_RATIO:-0.8}" \
    --seed "${QREG_SEED:-84}" \
    --alpha "${QREG_ALPHA:-1e-5}" \
    --regenerate
