...
class RunConfig:
    ...
    seed: int = 84
    alpha: float = 1e-5
    data_path: str = "data/cubic_regression_data.csv"
...
def quantum_features(x_scaled):
    ...
    encodings = np.array(
        [
            ...
            x_scaled**2,
            x_scaled**3,
            np.sin(np.pi * x_scaled),
        ],
        dtype=float,
    )
    for q, value in enumerate(encodings):
        state = apply_one_qubit_gate(state, ry(np.pi * value), q, n_qubits)
        state = apply_one_qubit_gate(state, rz(0.5 * np.pi * value), q, n_qubits)

    for control, target in [(0, 1), (1, 2), (2, 3), (3, 0)]:
        state = apply_cnot(state, control, target, n_qubits)

    for q, value in enumerate(encodings[::-1]):
        state = apply_one_qubit_gate(state, ry(0.35 * np.pi * value), q, n_qubits)

    features = [1.0, x_scaled, x_scaled**2, x_scaled**3]
    for q in range(n_qubits):
        features.append(expectation_z(state, q, n_qubits))
    for q0, q1 in [(0, 1), (1, 2), (2, 3), (0, 3), (0, 2), (1, 3)]:
        features.append(expectation_zz(state, q0, q1, n_qubits))
    return np.array(features, dtype=float)


def cubic_truth(x):
    return 0.65 * x**3 - 0.45 * x**2 + 0.85 * x + 0.25


def make_dataset(cfg):
    rng = np.random.default_rng(cfg.seed)
    x = np.linspace(-1.4, 1.4, cfg.n_points)
    y_clean = cubic_truth(x)
    y = y_clean + rng.normal(0.0, 0.09, size=cfg.n_points)
...
def scale_x(x, x_min=None, x_max=None):
    if x_min is None:
...
def fit_quantum_regressor(x_train, y_train, alpha):
    x_scaled, x_min, x_max = scale_x(x_train)
    phi = np.vstack([quantum_features(xi) for xi in x_scaled])
    regularizer = alpha * np.eye(phi.shape[1])
    weights = np.linalg.solve(phi.T @ phi + regularizer, phi.T @ y_train)
    return weights, x_min, x_max


def predict(x, weights, x_min, x_max):
    x_scaled, _, _ = scale_x(x, x_min, x_max)
    phi = np.vstack([quantum_features(xi) for xi in x_scaled])
    return phi @ weights
...
def plot_results(path, x, y, y_clean, split, weights, x_min, x_max, train_metrics, test_metrics):
    grid = np.linspace(float(np.min(x)), float(np.max(x)), 400)
    pred_grid = predict(grid, weights, x_min, x_max)
    ...
    plt.plot(grid, cubic_truth(grid), color="black", lw=2, ls="--", label="True cubic trend")
...
def main():
    parser = argparse.ArgumentParser(description="Quantum feature-map cubic regression")
    parser.add_argument("--n-points", type=int, default=100)
    parser.add_argument("--train-ratio", type=float, default=0.8)
    parser.add_argument("--seed", type=int, default=84)
    parser.add_argument("--alpha", type=float, default=1e-5)
    parser.add_argument("--data-path", default="data/cubic_regression_data.csv")
    ...
    train = split == "train"
    test = split == "test"
    weights, x_min, x_max = fit_quantum_regressor(x[train], y[train], cfg.alpha)
    pred_train = predict(x[train], weights, x_min, x_max)
    pred_test = predict(x[test], weights, x_min, x_max)

    train_metrics = metrics(y[train], pred_train)
    test_metrics = metrics(y[test], pred_test)

    os.makedirs(cfg.results_dir, exist_ok=True)
    plot_path = os.path.join(cfg.results_dir, "quantum_cubic_regression.png")
    plot_results(plot_path, x, y, y_clean, split, weights, x_min, x_max, train_metrics, test_metrics)

    np.savez(
        os.path.join(cfg.results_dir, "quantum_cubic_model.npz"),
        weights=weights,
        x_min=x_min,
        x_max=x_max,
        x=x,
    ...
    summary = {
        "model": "quantum cubic regression",
        "config": asdict(cfg),
        ...
        "outputs": {
            ...
            "plot": plot_path,
            "model": os.path.join(cfg.results_dir, "quantum_cubic_model.npz"),
        },
    }
    with open(os.path.join(cfg.results_dir, "quantum_cubic_summary.json"), "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    print("Quantum cubic regression complete")
