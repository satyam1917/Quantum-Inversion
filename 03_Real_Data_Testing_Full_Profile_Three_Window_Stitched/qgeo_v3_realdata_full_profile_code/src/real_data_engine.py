...
class RealDataConfig:
    ...
    fixed_window_reverse: str = "false"
...
def realstyle_radiometric_response(x_model_m, params):
    ...
    fault_x_m = float(p[0])
    peak_x_m = float(p[4])
    peak_width_m = max(float(p[5]), 50.0)
    peak_gain = max(float(p[6]), 0.0)
    contact_width_m = max(float(p[7]), 30.0)
    peak2_x_m = float(p[8]) if len(p) > 8 else fault_x_m + 2500.0
    peak2_width_m = max(float(p[9]), 50.0) if len(p) > 9 else 1200.0
    peak2_gain = max(float(p[10]), 0.0) if len(p) > 10 else 0.0
    trend_gain = float(p[11]) if len(p) > 11 else 0.0

    left_block = 1.0 - _sigmoid((x_model_m - fault_x_m) / contact_width_m)
    enrichment = np.exp(-0.5 * ((x_model_m - peak_x_m) / peak_width_m) ** 2)
    enrichment2 = np.exp(-0.5 * ((x_model_m - peak2_x_m) / peak2_width_m) ** 2)
    shoulder = np.exp(-0.5 * ((x_model_m - (fault_x_m + 1700.0)) / 800.0) ** 2)
    trend = (x_model_m - np.mean(x_model_m)) / (np.ptp(x_model_m) + 1e-12)

    shape = (
        0.45 * left_block
        + peak_gain * enrichment
        + peak2_gain * enrichment2
        + 0.10 * shoulder
        + trend_gain * trend
    )
...
def score_window(data, config, start_km, reverse):
    ...
    default_params = np.array([
        4500.0, 0.20, 0.15, -1200.0,
        1700.0, 800.0, 2.5, 450.0,
        6500.0, 1400.0, 0.7, 0.0,
    ])
...
def choose_fixed_window(data, config, start):
    choices = [False]
    reverse_setting = str(config.fixed_window_reverse).lower()
    if reverse_setting == "auto" and config.allow_reverse:
        choices = [False, True]
    elif reverse_setting in {"true", "1", "yes"}:
        choices = [True]

    rows = []
    best = None
    for reverse in choices:
        loss, dataset, ev = score_window(data, config, float(start), reverse)
        row = [
            float(loss),
            float(start),
...
def run_real_data_inversion(config):
    ...
    if str(config.window_start_km).lower() == "auto":
        ...
        dataset, scan_table = choose_fixed_window(data, config, start)
    ...
    for i in range(int(config.mc_samples)):
        ...
        perturb = np.array([
            180.0, 0.025, 0.025, 80.0,
            160.0, 90.0, 0.20, 60.0,
            220.0, 120.0, 0.20, 0.12,
        ])
