...
def run_complex_forward(
    ...
    # The synthetic benchmark uses 3000-7000 m, but the real Karnataka profile
    # places the strongest geophysical boundary close to the edge of the first
    # 10 km window. Keep a wider real-data-safe interval.
    fault_x_loc      = float(np.clip(fault_x_loc,      1500.0,   8000.0))
    rho_dense_scaled = float(np.clip(rho_dense_scaled,  1e-5,     5e-4))
    rho_basin_scaled = float(np.clip(rho_basin_scaled,  1e-5,     3e-4))
    dense_depth_top  = float(np.clip(dense_depth_top,  -2600.0,  -400.0))
    dense_depth_bot  = float(np.clip(dense_depth_top - 1000.0, -3400.0, -900.0))
